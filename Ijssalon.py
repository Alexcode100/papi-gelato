...
def WelkomZakelijk(Liters):
    ...
    LiterSmaak(Liter,Aardbei,Chocolade,Vanille,Liters)

def LiterSmaak(Liter,Aardbei,Chocolade,Vanille,Liters):
    AantalLiters = Liters
    while AantalLiters >=1:
        SmaakBol = input('Welke smaak wilt u voor de '+str(Liter)+'e liter? A) Aardbei, C) Chocolade, of V) Vanille?\n---> ')
        if SmaakBol == "A":
            Aardbei = Aardbei + 1
        elif SmaakBol == "C":
...
def AantalBolletjes(Aantal_Bolletjes):
    while True:
        try:  
            ...
            print('Sorry dat is geen optie die we aanbieden...')
    SmaakBolletjes(Aantal_Bolletjes,Bolletje,Aardbei,Chocolade,Vanille)

def SmaakBolletjes(Aantal_Bolletjes,Bolletje,Aardbei,Chocolade,Vanille):
    Aantalx = Aantal_Bolletjes
    while Aantalx >= 1:
        SmaakBol = input('Welke smaak wilt u voor bolletje nummer '+str(Bolletje)+'? A) Aardbei, C) Chocolade, of V) Vanille?\n---> ')
        Bolletje = Bolletje + 1
        Aantalx = Aantalx - 1
        if SmaakBol == "A":
            Aardbei = Aardbei + 1
...
def Bonnetje(Aantal_Bolletjes,Hoorntje,Bakje,ToppingKosten):
    PrijsBolletjes = Aantal_Bolletjes*0.95
